...
@export var nb_item_ligne: int
@export var nb_item_max: int
@export var x_initial: int
@export var y_initial: int
@export var taille_cadre: int
@export var taille_objet: int
@export var x_initial_cadre: int
@export var y_initial_cadre: int
@export var distance_objet: int
...
func ajout_cadres():
	...
	for i in liste_objets.size():
		if liste_objets[i] == null:
			cadre = cadres[0].instantiate()
		else:
			var niveau = liste_objets[i].niveau
			cadre = cadres[niveau].instantiate()
		add_child(cadre)
...
func ajout_generateur():
	var bouton = generateur.instantiate()
	add_child(bouton)
